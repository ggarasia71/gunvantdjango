...
from .models import Contact,User,Product,Wishlist,Cart,Order
...
def contact(request):
	if request.method=="POST":
		...
		msg="Contact Saved Successefully"
...
def signup(request):
	if request.method=="POST":
		try:
			...
			msg="Email is Aready Registered"
			...
			if request.POST['password']==request.POST['cpassword']:
				...
				msg="User Sign Up Successefully"
				return render(request,'login.html',{'msg':msg})
			else:
...
def edit_profile(request):
	user=User.objects.get(email=request.session['email'])
	print(user.fname,user.email,user.mobile)
	if request.method=="POST":
...
def seller_add_product(request):
	if request.method=="POST":
		...
		msg="Product Added Successefully"
...
def seller_search_product(request):
	if request.method=="POST":
		products=Product.objects.filter(product_category=request.POST['search'])
		return render(request,'seller_search_product.html',{'products':products})
	else:	
		return render(request,'seller_search_product.html')
def add_to_wishlist(request,pk):
	product=Product.objects.get(pk=pk)
	user=User.objects.get(email=request.session['email'])
	Wishlist.objects.create(product=product,user=user)
	# return redirect('products') redirect to product,b'cos wishlist created later on
